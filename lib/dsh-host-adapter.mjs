...
export function createDshHostAdapter(typertGateway) {
  ...
  // DSH renamed the `commands/execute` attachment field: 0.1.2-rc.1 and earlier
  // take `images`, 0.1.5-rc.1 takes `submittedAttachments`. The Typert descriptor
  // validates the argument set strictly and rejects an unknown field outright
  // ("args fields do not match the descriptor: missing …; unexpected …"), so the
  // field cannot simply be sent twice. Probe once, remember what the host
  // accepted, and fall back transparently so one adapter serves both versions.
  let attachmentField = 'submittedAttachments'
  const attachmentFieldMismatch = (error, field) => {
    const code = error && error.code ? String(error.code) : ''
    const message = error && error.message ? String(error.message) : ''
    return (code.includes('arguments-invalid') || message.includes('arguments-invalid'))
      && message.includes(field)
  }

  const commands = {
    list: (sessionId, signal) => invoke('commands', 'list', { agentId: sessionId }, signal),
    execute: async (sessionId, line, images, signal) => {
      const attempt = (field) => invoke(
        'commands',
        'execute',
        { agentId: sessionId, line, [field]: images },
        signal,
      )
      try {
        return await attempt(attachmentField)
      } catch (error) {
        const alternative = attachmentField === 'images' ? 'submittedAttachments' : 'images'
        if (!attachmentFieldMismatch(error, attachmentField)) throw error
        const result = await attempt(alternative)
        attachmentField = alternative
        return result
      }
    },
  }
