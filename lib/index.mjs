...
// Project one assistant chunk (LLM delta) into the mobile wire shape. Used by
// both ingestion paths: the durable `session/event` feed (DSH <= 0.1.2) and the
// process-local `agent/assistant-stream` channel (DSH 0.1.5+).
function assistantChunkEvent(turn, step, chunk) {
  const c = chunk || {}
  const ev = { type: 'assistant/chunk', turn, step, chunkType: c.type }
  if (c.type === 'text-delta' || c.type === 'reasoning-delta') ev.text = c.text
  if (c.type === 'tool-call-delta') ev.tool = { id: c.id, name: c.name, argumentsDelta: c.argumentsDelta }
  if (c.type === 'usage') ev.usage = c.usage
  if (c.type === 'finish') ev.finish = { kind: c.reason && c.reason.kind }
  return ev
}
...
function buildWireEvent(session, event) {
  ...
  switch (event.type) {
    ...
    case 'assistant/chunk':
      return Object.assign(base, { event: assistantChunkEvent(d.turn, d.step, d.chunk) })
...
const plugin = {
  ...
  apply(ctx, config) {
    ...
    // DSH 0.1.5 stopped emitting `assistant/chunk` on `session/event`: live
    // assistant output moved to the process-local `agent/assistant-stream`
    // channel that the WebUI's session followers consume. Projecting those
    // frames into the existing wire shape keeps the mobile protocol (and the iOS
    // client) unchanged, and the listener is simply never called on older hosts.
    const disposeAssistantStream = ctx.on('agent/assistant-stream', ({ agent, frame }) => {
      if (clients.size === 0) return
      if (!frame || frame.type !== 'chunk' || !frame.chunk) return
      ...
      const wire = {
        kind: 'event',
        sessionId,
        ...(Number.isSafeInteger(session.seq) ? { seq: session.seq } : {}),
        time: frame.time,
        event: assistantChunkEvent(frame.turn, frame.step, frame.chunk),
      }
      const payload = JSON.stringify(wire)
      for (const client of clients) {
        if (client.mobileChannel === 'control') continue
        if (client.filterSessionId && client.filterSessionId !== sessionId) continue
        if (client.readyState === 1) client.send(payload)
      }
    })
    log('agent/assistant-stream listener attached (DSH 0.1.5+ live chunks)')
