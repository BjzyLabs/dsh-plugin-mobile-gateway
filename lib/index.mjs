...
const Config = Schema.object({
  ...
  // ws:// is always allowed on loopback, RFC 1918 and .local. The Tailscale
  // overlay (RFC 6598 addresses, MagicDNS .ts.net names) is a private network
  // too, but accepting it as plaintext ws:// is a deployment decision, not a
  // default: enable this only when the tailnet — and its ACLs — are the access
  // boundary you intend. WireGuard still encrypts the transport, and device
  // authentication is unaffected. Leave false to require wss:// there.
  allowTailnetTransport: Schema.boolean().default(false),
})
...
// Tailscale hands every node an address from the RFC 6598 shared range
// (100.64.0.0/10) and a MagicDNS name ending in .ts.net. Both are reachable
// only by tailnet members, and the suffix is matched on the final labels so a
// lookalike public domain such as "host.ts.net.example.com" stays public.
...
// ws:// is permitted only on transports the operator has declared private:
// loopback / RFC 1918 / .local always, plus the Tailscale overlay when
// `allowTailnetTransport` is explicitly enabled.
function isAllowedOverlayHostname(hostname, allowTailnet) {
  return isPrivateNetworkHostname(hostname) || (allowTailnet === true && isTailnetHostname(hostname))
}

function privateLanAddresses(allowTailnet) {
  const physical = []
  const fallback = []
  ...
  for (const [name, records] of Object.entries(os.networkInterfaces())) {
    for (const record of records || []) {
      if (!record || record.internal || record.family !== 'IPv4' || !isAllowedOverlayHostname(record.address, allowTailnet)) continue
...
function lanWebSocketUrls(options, wsPath, boundPort) {
  ...
  const hosts = advertised
    ...
    : options.lanHost && options.lanHost !== '0.0.0.0' && options.lanHost !== '::'
      ? [options.lanHost]
      : privateLanAddresses(options.allowTailnetTransport === true)
  return hosts
    .filter((host) => isAllowedOverlayHostname(host, options.allowTailnetTransport === true))
...
function normalizePublicUrl(value, req, wsPath, allowTailnet) {
  ...
  if (url.protocol === 'ws:' && !isAllowedOverlayHostname(url.hostname, allowTailnet)) {
    const hint = allowTailnet !== true && isTailnetHostname(url.hostname)
      ? ' — that is a Tailscale address; set allowTailnetTransport: true to allow ws:// on the tailnet'
      : ''
    throw badRequest(`publicUrl must use wss:// outside localhost or a private LAN${hint}`)
  }
  return url.toString()
}
...
const plugin = {
  ...
  apply(ctx, config) {
    ...
    if (options.lanEnabled === true && options.lanAdvertiseHost
        && !isAllowedOverlayHostname(options.lanAdvertiseHost, options.allowTailnetTransport === true)) {
      throw new Error('mobile-gateway: lanAdvertiseHost must be localhost, a private LAN address, or a Tailscale address with allowTailnetTransport: true')
    }
    ...
    if (options.allowTailnetTransport === true) {
      log('allowTailnetTransport enabled: plaintext ws:// is accepted on Tailscale addresses and MagicDNS names. '
        + 'This assumes the tailnet is the access boundary — a peer must be a tailnet member AND permitted by its ACLs to reach this port; '
        + 'the transport is WireGuard-encrypted; device authentication is still required.')
    }
        ...
        try {
          ...
          if (req.method === 'GET' && p === '/mgw/status') {
            sendJson(res, 200, {
              ...
              // Echoes the config key, so the status field and the setting an
              // operator edits are the same word. True means: this gateway
              // accepts plaintext ws:// from Tailscale addresses and MagicDNS
              // names instead of requiring wss:// there. It says nothing about
              // Tailscale's own permissions — those are the tailnet's ACLs.
              allowTailnetTransport: options.allowTailnetTransport === true,
            })
            ...
            const publicUrl = normalizePublicUrl(body.publicUrl || configuredPublicUrl(), req, wsPath, options.allowTailnetTransport === true)
    ...
    if (options.lanEnabled === true) {
      ...
      lanServer.on('upgrade', (req, socket, head) => {
        ...
        if (!req.socket || !req.socket.remoteAddress
            || !isAllowedOverlayHostname(req.socket.remoteAddress, options.allowTailnetTransport === true)) {
